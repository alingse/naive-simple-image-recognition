...
def remove_points(im):
    nim=Image.new('RGB',im.size)
    col,row=im.size
    for i in range(1,col-1): 
        for j in range(1,row-1):
            t=im.getpixel((i,j))
            tl=im.getpixel((i-1,j))
            tr=im.getpixel((i+1,j))
            tt=im.getpixel((i,j-1))
            tp=im.getpixel((i,j+1))
            nim.putpixel((i,j),t)
            if t[0]==255 and sum(tl+tr+tt+tp)==0:
                nim.putpixel((i,j),(0,0,0))
            if t[0]==0 and sum(tl+tr+tt+tp)==3*4*255:
                nim.putpixel((i,j),(255,255,255))
